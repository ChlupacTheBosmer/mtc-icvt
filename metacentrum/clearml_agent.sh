...
singularity exec -B $HOMEDIR:/mnt \
$SING_IMAGE /bin/bash -c "pip install clearml clearml-agent; source ~/.bashrc; export PATH=$PATH:$HOMEDIR.local/bin; clearml-agent config; clearml-agent daemon --queue default"
